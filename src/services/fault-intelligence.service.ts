import type { Airport, Equipment, Fault, FaultPriority, Inspection, Repair, Terminal, Floor, Zone } from "@/lib/types";
...
import { listAirports, listAllTerminals, listAllFloors, listAllZones } from "./airports.service";
...
async function fetchAll(): Promise<RawData> {
  const [faultsPage, repairsPage, equipmentPage, inspectionsPage, airports, terminals, floors, zones] =
    await Promise.all([
...
export async function getHealthSummary(scope?: { airportId?: number }): Promise<HealthSummary> {
  ...
  const scopedZones = raw.zones.filter((z) => {
    const floor = raw.floors.find((f) => f.id === z.floorId);
    return floor ? scopedTerminals.some((t) => t.id === floor.terminalId) : false;
  });
  ...
  const zoneEntities = scopedZones.map((z) => {
    const floor = raw.floors.find((f) => f.id === z.floorId)!;
    const terminal = raw.terminals.find((t) => t.id === floor.terminalId)!;
    return buildEntity(
      raw,
      "zone",
      `zone-${z.id}`,
