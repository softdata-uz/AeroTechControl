...
import type { EquipmentType, Regulation } from "@/lib/types";
...
/** Create/edit modal for Regulation — name plus the parent Equipment Type. Mounted only while open. */
export function RegulationFormModal({ onClose, onSaved, types, initial, create, update }: Props) {
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={`${t(initial ? "settingsCrud.editPrefix" : "equipment.lookupModal.newPrefix")} ${t("settingsCrud.regulationLabel")}`}
      footer={
        <>
    ...
    >
      <div className="space-y-4">
        ...
        />
        <Input
          label={t("settingsCrud.regulationName")}
          required
          value={name}
