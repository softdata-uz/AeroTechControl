...
export default function SettingsLocationsPage() {
  ...
  const { airports, terminals, zones, loading, refetch } = useLocations();
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ kind: "airport" | "terminal" | "zone"; id: number } | null>(
    null
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 px-6 pb-6 pt-5 lg:grid-cols-3">
        <CrudListPanel
          title={t("settings.airports")}
          loading={loading}
          addLabel={t("equipment.form.airport")}
          items={airports.map((a) => ({ id: a.id, primary: a.city, secondary: `${a.name} · ${a.code}` }))}
          onAdd={() => {
        ...
        <CrudListPanel
          ...
          items={zones.map((z) => ({
            id: z.id,
            primary: z.name,
            secondary: terminalName(z.terminalId),
          }))}
          onAdd={() => {
            setEditingZone(null);
